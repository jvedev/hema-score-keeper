import css from "./stage-view.css?raw";
...
let stageRoot: ShadowRoot;
let activeContext: StageContext | undefined;

function activateContext(context: StageContext): void {
  activeContext = context;
  state = context.state;
  stageRoot = context.root;
  noticeTimer = context.noticeTimer;
}
...
function render(): void {
  ...
  stageRoot.innerHTML = `<style>${css}</style>
    <div class="shell">
      <header class="topbar">
        <div class="title">
...
function flash(message: string): void {
  ...
  noticeTimer = window.setTimeout(() => {
    ...
    context.noticeTimer = undefined;
    render();
  }, 1200);
  context.noticeTimer = noticeTimer;
}
...
export class StageView extends BaseComponent {
  ...
  connectedCallback(): void {
    ...
    this.registerEvent<MouseEvent>(this.root, "click", (event) =>
      this.#run(() => handleClick(event)),
    );
    this.registerEvent(this.root, "change", (event) =>
      this.#run(() => handleChange(event)),
    );
    this.registerEvent(this.root, "input", (event) =>
      this.#run(() => handleInput(event)),
    );
    this.registerEvent<SubmitEvent>(this.root, "submit", (event) =>
      this.#run(() => handleSubmit(event)),
    );
    this.registerEvent<DragEvent>(this.root, "dragstart", (event) =>
      this.#run(() => handleDragStart(event)),
    );
    this.registerEvent<DragEvent>(this.root, "dragover", (event) =>
      this.#run(() => handleDragOver(event)),
    );
    this.registerEvent<DragEvent>(this.root, "drop", (event) =>
      this.#run(() => handleDrop(event)),
    );
    this.#run(render);
  }
  ...
  override disconnectedCallback(): void {
    ...
    if (context?.noticeTimer !== undefined) {
      window.clearTimeout(context.noticeTimer);
      context.noticeTimer = undefined;
    }
...
if (!customElements.get("stage-view")) {
  customElements.define("stage-view", StageView);
}
